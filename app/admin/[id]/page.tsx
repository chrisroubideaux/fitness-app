...
export default function AdminPage() {
  ...
  useEffect(() => {
    // ✅ Standardize storage key for admin JWT
    const tokenFromURL = searchParams.get('token');
    if (tokenFromURL) {
      localStorage.setItem('adminAuthToken', tokenFromURL);
      // (optional) avoid cross-contamination:
      // localStorage.removeItem('authToken');
    }

    const token = tokenFromURL || localStorage.getItem('adminAuthToken');
    if (!token) {
      setError('No token found. Please log in again.');
      setLoading(false);
      return;
  ...
  const handleLogout = () => {
    localStorage.removeItem('adminAuthToken'); // ✅ same key
    router.push('/admin');
  };
  ...
  if (loading) {
    return <div style={{ padding: '2rem' }}><h2>Loading admin profile...</h2></div>;
  }
