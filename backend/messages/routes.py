...
def _serialize_conversation(c: Conversation, viewer_kind: str):
    return {
        ...
        "unread_count": c.user_unread_count if viewer_kind == "user" else c.admin_unread_count,
...
def delete_conversation(conversation_id):
    """Either participant may delete (adjust if you want admin-only)."""
    try:
        kind, me = resolve_principal()
    except AuthError as e:
        return jsonify({"error": "Unauthorized", "message": str(e)}), 401
...
def delete_message(message_id):
    """Sender can delete within grace; admin participant can always delete (moderation)."""
    try:
        kind, me = resolve_principal()
    except AuthError as e:
        return jsonify({"error": "Unauthorized", "message": str(e)}), 401
...
def _serialize_message(m: Message):
    return {
        ...
        "read_by_user_at": m.read_by_user_at.isoformat() if m.read_by_user_at else None,
        "read_by_admin_at": m.read_by_admin_at.isoformat() if m.read_by_admin_at else None,
    }
...
def _get_or_create_conversation(user_id, admin_id) -> Conversation:
    ...
    if conv:
        return conv
    conv = Conversation(user_id=user_id, admin_id=admin_id, last_message_at=datetime.utcnow())
    db.session.add(conv)
    db.session.commit()
    return conv
...
def edit_message(message_id):
    ...
    if datetime.utcnow() - msg.created_at > EDIT_GRACE:
        return jsonify({"error": "Edit window expired"}), 400
...
def delete_message(message_id):
    ...
    within_window = datetime.utcnow() - msg.created_at <= EDIT_GRACE
...
def mark_read(conversation_id):
    ...
    db.session.commit()
    return jsonify({"ok": True, "read_at": now.isoformat()}), 200
