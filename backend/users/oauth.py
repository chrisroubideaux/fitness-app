...
# === GOOGLE REDIRECT FLOW: Step 2 (callback) ===
...
def handle_google_callback():
    ...
    if not user:
        user = User(id=str(uuid4()), email=email, profile_image_url=picture)
        db.session.add(user)
        db.session.commit()

    jwt_token = generate_jwt_token(user.id, user.email)

    # Redirect to frontend with token (optional — change as needed)
    return redirect(f"http://localhost:3000/profile/{user.id}?token={jwt_token}")
