          ...
          <div className="card-body">
            <div className="row g-3">
              ...
              <div className="col-sm-6">
                ...
                <div className="fw-semibold">{nextCharge}</div>
              </div>
...
type Summary = {
  ...
  next_bill: {
    amount: number | null;
    currency: string | null;
    date_unix: number | string | null; // seconds (or string)
    date_iso: string | null;
  } | null;
  status?: string | null;
  cancel_at_period_end?: boolean;
  // Optional debugging fields your backend can include
  debug?: { current_period_end?: number; upcoming_period_end?: number; sub_id?: string };
};
...
type Me = {
  ...
  membership_plan_id: string | null;
  plan_name?: string | null;
  plan_price?: number | null; // dollars
  plan_features?: string[];
};
...
export default function SubscriptionDetails({
  ...
  const debug =
    ...
    localStorage.getItem("showSummaryDebug") === "1";
  ...
  async function load() {
    ...
    try {
      ...
      if (sumRes.ok) {
        if (debug) {
          try {
            console.log("[/summary response]", sumJson);
          } catch {}
        }
        setSummary(sumJson as Summary);
      }
  ...
  // If subscription is active but date hasn’t appeared yet, soft-retry once.
  useEffect(() => {
    const s = summary;
    if (!s) return;
    const activeish =
    ...
    const hasIso = !!s.next_bill?.date_iso;
    const hasUnix =
      s.next_bill?.date_unix != null &&
      `${s.next_bill.date_unix}`.trim() !== "";
    const missingDate = !hasIso && !hasUnix;
    if (activeish && missingDate) {
      const t = setTimeout(load, 1500);
      return () => clearTimeout(t);
    }
  }, [summary]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // Robust: accepts ISO or UNIX (string/number), auto-detects sec vs ms
  function formatDate(iso?: string | null, unixMaybe?: number | string | null) {
    if (iso) {
      try {
    ...
    if (unixMaybe != null && `${unixMaybe}`.trim() !== "") {
      const n = Number(unixMaybe);
      if (!Number.isNaN(n)) {
        const ms = n > 10_000_000_000 ? n : n * 1000; // seconds → ms if needed
        const d = new Date(ms);
  ...
  // Prefer Stripe upcoming total; fallback to app plan price so users always see a number.
  const nextCharge =
    formatMoneyCents(
      summary?.next_bill?.amount ?? null,
      summary?.next_bill?.currency ?? undefined
    ) ?? (me?.plan_price != null ? `${formatMoneyDollars(me.plan_price)} / mo` : "—");

  // Also look for backend debug fallbacks (if present)
  const debugAny: Partial<Summary> = summary || {};
  const debugUnix =
    debugAny?.debug?.current_period_end ?? debugAny?.debug?.upcoming_period_end ?? null;

  // Use ISO if present; else UNIX; else debug UNIX
  const nextBillDate = formatDate(
    summary?.next_bill?.date_iso ?? null,
    summary?.next_bill?.date_unix ?? debugUnix
  );
  ...
  const visiblePerks = showAllPerks
    ? uniquePerks
    : uniquePerks.slice(0, PREVIEW_COUNT);
  const hasOverflow = totalPerks > PREVIEW_COUNT;
  ...
  return (
    <div id="panel-details" role="tabpanel" aria-labelledby="tab-details">
        ...
        <div className="card shadow-sm" style={{ borderRadius: 16 }}>
          <div className="card-body">
            <div className="row g-3">
              <div className="col-sm-6">
                <div className="text-uppercase text-muted small">Plan</div>
                <div className="fw-semibold">{resolvedPlanName(summary, me)}</div>
              ...
              <div className="col-sm-6">
                ...
                {debug && (
                  <div className="small text-muted mt-1">
                    raw iso: {String(summary?.next_bill?.date_iso ?? "null")} · raw unix:{" "}
                    {String(summary?.next_bill?.date_unix ?? "null")} · debug unix:{" "}
                    {String(debugUnix ?? "null")}
                  </div>
                )}
              </div>
            </div>
      ...
      {debug && (
        <pre className="small mt-3 bg-light p-2 rounded">
